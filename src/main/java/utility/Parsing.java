...
public class Parsing implements Serializable {
	...
	public static int[] getIntArrayFromCsvFile(String filename, String fieldName) {
		...
		try {
			...
			// read the header record and get the column index that matches the fieldName
			// passed in
	...
	/**
	 * Parses a csv file and returns a map keyed by keyField.
	 * 
	 * @param filePath
	 *            - the full path to the input file
	 * @param keyField
	 *            - the key field name for the returned map, Integer values
	 * @param valueFields
	 *            - the List of value field names for the returned map, Integer
	 *            values
	 * @return a map with key value pairs
	 */
	public static Map<Integer, List<String>> parseCsvFileFieldsToMap(String filePath) {

		Map<Integer, List<String>> returnValuesMap = new TreeMap<>();

		try (Scanner sc = new Scanner(new File(filePath))) {
			String[] record;
			String[] headerFieldNames = null;

			// read header record field names
			headerFieldNames = sc.nextLine().split(",");

			// use the first field as the key field - assumed to be integer field of ids, starting at 1.
			int keyFieldIndex = 0;
			List<Integer> valueFieldIndices = new ArrayList<>();
			for (String field : headerFieldNames)
				valueFieldIndices.add(getFieldIndex(field, headerFieldNames));

			returnValuesMap.put(0, Arrays.asList(headerFieldNames));

			while (sc.hasNextLine()) {
				record = sc.nextLine().split(",", -1);
				List<String> values = new ArrayList<>();

				for (int valueIndex : valueFieldIndices) {
					if (valueIndex < 0 || valueIndex >= record.length)
						throw new RuntimeException("invalid index, valueIndex=" + valueIndex + ", valueFieldIndices=" + valueFieldIndices);
					values.add(record[valueIndex]);
				}
				returnValuesMap.put(Integer.valueOf(record[keyFieldIndex]), values);
			}
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
	...
	private static int getFieldIndex(String field, String[] fieldNames) {
		int index = -1;
		for (int i = 0; i < fieldNames.length; i++) {
			if ( fieldNames[i].equalsIgnoreCase(field) ) {
				index = i;
				break;
			}
		}
